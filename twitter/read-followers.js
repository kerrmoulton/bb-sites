/* @meta
{
  "name": "twitter/read-followers",
  "description": "Private BB read adapter: Get accounts following a Twitter/X user (defaults to the logged-in user when no user is given)",
  "domain": "x.com",
  "readOnly": true,
  "args": {
    "request": {
  ...
  "example": "bb-browser site twitter/read-followers --json"
}
*/
...
var XRead = (() => {
  ...
  // src/upstream/followers.js
  var FOLLOWING_QUERY_ID = "sF7aRC2fRq7OGOOp_qHntA";
  ...
  function buildFollowersUrl(queryId, userId, count, cursor) {
    const vars = {
      userId,
      count,
    ...
    return `/i/api/graphql/${queryId}/Followers?variables=${encodeURIComponent(JSON.stringify(vars))}&features=${encodeURIComponent(JSON.stringify(FEATURES))}`;
  }
  function extractUser(result) {
    ...
    if (!screenName) {
      throw new CommandExecutionError("Malformed Twitter followers user: missing screen_name");
    }
    return {
      screen_name: screenName,
  ...
  cli({
    ...
    name: "followers",
    access: "read",
    description: "Get accounts a Twitter/X user is followers (defaults to the logged-in user when no user is given)",
    domain: "x.com",
    ...
    args: [{
      ...
      type: "int",
      default: 50,
      help: "Maximum number of followers rows to return (default 50). Must be a positive integer."
    }],
    columns: ["screen_name", "name", "bio", "followers"],
    func: async (page, kwargs) => {
      const limit = kwargs.limit === void 0 || kwargs.limit === null ? 50 : Number(kwargs.limit);
      if (!Number.isInteger(limit) || limit <= 0) {
        throw new ArgumentError("twitter followers --limit must be a positive integer", "Example: opencli twitter followers @elonmusk --limit 200");
      }
      const rawUser = String(kwargs.user ?? "").trim();
      let targetUser = normalizeScreenName(rawUser);
      if (rawUser && !targetUser) {
        throw new ArgumentError("twitter followers user must be a valid Twitter/X handle", "Example: opencli twitter followers @elonmusk --limit 200");
      }
      ...
      if (!targetUser) {
        throw new ArgumentError("twitter followers user cannot be empty", "Example: opencli twitter followers @elonmusk --limit 200");
      }
      const followersQueryId = typeof findGraphQLQueryId === "function" ? findGraphQLQueryId("Followers", FOLLOWING_QUERY_ID) : FOLLOWING_QUERY_ID;
      ...
      for (let i = 0; i < MAX_PAGINATION_PAGES && allUsers.length < limit; i++) {
        ...
        const apiUrl = buildFollowersUrl(followersQueryId, userId, fetchCount, cursor);
        const data = unwrapBrowserResult(await page.evaluate(async (url, headers2) => {
          const gen = typeof findTransactionIdGenerator === "function" ? await findTransactionIdGenerator() : null;
          if (!gen) throw new CommandExecutionError("TRANSACTION_ID_GENERATOR_UNAVAILABLE");
          headers2 = { ...headers2, "X-Client-Transaction-Id": await gen("x.com", new URL(url, location.href).pathname, "GET") };
        ...
        if (data?.error) {
          if (data.error === 401 || data.error === 403) throw new AuthRequiredError("x.com", `Twitter followers request failed (HTTP ${data.error})`);
          throw new CommandExecutionError(describeTwitterApiError("Followers", data.error));
        }
      ...
      if (allUsers.length === 0) {
        if (looksLikePrivateTwitterTimeline(lastRawResponse)) {
          throw new EmptyResultError("twitter followers", `No followers data returned for @${targetUser} (the target account may have set their followers list to private)`);
        }
        throw new EmptyResultError("twitter followers", `No followers accounts found for @${targetUser}`);
      }
...
return XRead.runRead("followers",args);
}
