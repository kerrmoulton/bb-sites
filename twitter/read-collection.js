/* @meta
{
  "name": "twitter/read-collection",
  "description": "Private BB read adapter: Fetch a user timeline with relationship facts and a bounded completion receipt.",
  "domain": "x.com",
  "readOnly": true,
  "args": {
    "request": {
  ...
  "example": "bb-browser site twitter/read-collection --json"
}
*/
...
var XRead = (() => {
  ...
  var DEFAULT_USER_TWEETS_PAGE_DELAY_SECONDS = 2;
  ...
  async function resolveUserTimelineContext(page, rawUsername, {
    ...
    if (raw && !username) {
      throw new ArgumentError(`twitter ${commandName} username must be a valid Twitter/X handle`, commandName === "collection" ? "Example: opencli twitter collection @jack --until 2026-07-23T00:00:00Z" : "Example: opencli twitter tweets @jack --limit 20");
    }
    ...
    const userId = unwrapBrowserResult(await ((__bb_static_arg_0, __bb_static_arg_1) => page.evaluate(() => {
      const __bb_value = async () => {
      ...
      return typeof __bb_value === "function" ? __bb_value() : __bb_value;
    }))(userByScreenNameUrl, JSON.parse(headers)));
  ...
  async function fetchUserTimelinePage(page, context, cursor, count) {
    const url = buildUserTweetsUrl(context.userTweetsOperation, context.userId, count, cursor);
    return normalizeTwitterGraphqlPayload(await ((__bb_static_arg_0, __bb_static_arg_1) => page.evaluate(() => {
      const __bb_value = async () => {
        const response = await fetch(__bb_static_arg_0, {
          headers: __bb_static_arg_1,
          credentials: "include"
        });
      ...
      return typeof __bb_value === "function" ? __bb_value() : __bb_value;
    }))(url, JSON.parse(context.headers)));
  }

  // src/upstream/collection.js
  ...
  function relationshipTarget(result, fallbackId = null, contextStatus = "complete") {
    ...
    const authorHandle = typeof rawHandle === "string" && normalizeTwitterScreenName(rawHandle) ? normalizeTwitterScreenName(rawHandle) : null;
    ...
    const resolvedContextStatus = contextStatus === "complete" && !hasVisibleContext ? postId ? "unavailable" : "unknown" : contextStatus;
    return {
      post_id: postId,
  ...
  function extractRelationship(result) {
    ...
    const replyAuthorId = typeof legacy.in_reply_to_user_id_str === "string" && legacy.in_reply_to_user_id_str.trim() ? legacy.in_reply_to_user_id_str : null;
...
return XRead.runRead("collection",args);
}
