/* @meta
{
  "name": "twitter/read-likes",
  "description": "Private BB read adapter: Fetch liked tweets of a Twitter user (defaults to the logged-in user when no username is given)",
  "domain": "x.com",
  "readOnly": true,
  "args": {
    "request": {
  ...
  "example": "bb-browser site twitter/read-likes --json"
}
*/
...
var XRead = (() => {
  ...
  function readResumeFile(filePath, expected = null) {
    ...
    if (expected) {
      ...
      if (String(parsed.username || "").toLowerCase() !== String(expected.username).toLowerCase()) throw new ArgumentError(`Resume file username mismatch: expected @${expected.username}, found @${parsed.username || "unknown"}`);
      ...
      if (!expected.outputFile && !Array.isArray(parsed.tweets)) throw new CommandExecutionError(`Twitter likes resume file ${filePath} is missing in-memory tweets`);
      if (!expected.outputFile && parsed.tweets.length !== count) throw new CommandExecutionError(`Twitter likes resume file ${filePath} count does not match its in-memory tweets`);
      if (parsed.complete) throw new CommandExecutionError(`Twitter likes resume file ${filePath} is already marked complete`);
    }
    return {
      cursor,
      count,
  ...
  cli({
    ...
    args: [{
      ...
      type: "int",
      default: 20,
      help: "Maximum number of liked tweets to return (default 20). Ignored when --all is set."
    }, {
      name: "all",
      type: "bool",
      default: false,
      help: "Fetch all liked-tweet pages until exhausted. Prefer --output-file for large archives."
    }, {
      name: "resume-file",
      type: "string",
      help: "Resume file for long-running all-pages likes syncs."
    }, {
      name: "output-file",
      type: "string",
      ...
      type: "int",
      default: 0,
      help: "When set to N>0, re-rank the liked tweets by weighted engagement (likes\xD71 + retweets\xD73 + replies\xD72 + bookmarks\xD75 + log10(views+1)\xD70.5) and return the top N. Default 0 keeps the API's native (recency) ordering. Incompatible with --output-file."
    }],
    ...
    func: async (page, kwargs) => {
      ...
      const userId = unwrapBrowserResult(await ((__bb_static_arg_0, __bb_static_arg_1, __bb_static_arg_2) => page.evaluate(() => {
        const __bb_value = async () => {
          const screenName = __bb_static_arg_0;
          const url = __bb_static_arg_1;
        ...
        return typeof __bb_value === "function" ? __bb_value() : __bb_value;
      }))(username, buildUserByScreenNameQueryUrl(userByScreenNameQueryId, username), JSON.parse(headers)));
      if (!userId) {
      ...
      while (pages < maxPages && (fetchAll || allTweets.length < limit)) {
        ...
        if (data?.error) {
          if ((useOutputFile ? outputCount : allTweets.length) === 0) throw new CommandExecutionError(describeTwitterApiError("Likes", data.error));
          break;
        }
        lastRawResponse = data;
        const hasInstructions = Array.isArray(data?.data?.user?.result?.timeline_v2?.timeline?.instructions) || Array.isArray(data?.data?.user?.result?.timeline?.timeline?.instructions);
        if (!hasInstructions) {
      ...
      if (fetchAll && !exhausted) {
        throw new CommandExecutionError(`twitter_likes_archive_incomplete: stopped after ${pages} page(s); completion cannot be proven`, resumeFile ? `Resume with --resume-file ${resumeFile}` : "Rerun with --resume-file to preserve continuation state.");
      }
...
return XRead.runRead("likes",args);
}
