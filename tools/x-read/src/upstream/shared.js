...
const RESERVED_SCREEN_NAME_PATHS = new Set(['compose', 'explore', 'help', 'home', 'i', 'intent', 'jobs', 'login', 'logout', 'messages', 'notifications', 'privacy', 'search', 'settings', 'signup', 'tos']);
...
export async function resolveTwitterOperationMetadata(page, operationName, fallback) {
  ...
  const resolved = await ((__bb_static_arg_0, __bb_static_arg_1) => page.evaluate(() => {
    const __bb_value = async () => {
      const operationName = __bb_static_arg_0;
      ...
      const parseOperationFromBundleText = __bb_static_arg_1;
      try {
...
export function extractCard(tweet) {
  ...
  const matchingEntity = cardUrlBinding ? urlEntities.find(entity => entity?.url === cardUrlBinding || entity?.expanded_url === cardUrlBinding) : undefined;
