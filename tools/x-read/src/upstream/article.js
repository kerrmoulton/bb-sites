  ...
  func: async (page, kwargs) => {
    ...
    const rawResult = unwrapBrowserResult(await ((__bb_static_arg_0, __bb_static_arg_1, __bb_static_arg_2, __bb_static_arg_3) => page.evaluate(() => {
      const __bb_value = async () => {
        const tweetId = __bb_static_arg_0;
        const ct0 = __bb_static_arg_1;
        ...
        if (typeof returnedTweetId !== 'string' || returnedTweetId !== tweetId) {
          return {
            error: 'Twitter API response did not match requested tweet ' + tweetId
          };
        }
        ...
        if (typeof screenName !== 'string' || !/^[A-Za-z0-9_]{1,15}$/.test(screenName)) {
          return {
            error: 'Twitter API response did not include a valid author screen name for tweet ' + tweetId
          };
        }
        ...
        if (!articleResults) {
          ...
          return {
            error: 'Tweet ' + tweetId + ' has no article content'
          };
        }
        ...
        for (const block of blocks) {
          ...
          if (blockType === 'header-one') parts.push('# ' + text);else if (blockType === 'header-two') parts.push('## ' + text);else if (blockType === 'header-three') parts.push('### ' + text);else if (blockType === 'blockquote') parts.push('> ' + text);else if (blockType === 'unordered-list-item') parts.push('- ' + text);else if (blockType === 'ordered-list-item') {
            orderedCounter++;
