      ...
      const data = throwIfLoginWall(await ((__bb_static_arg_0, __bb_static_arg_1) => page.evaluate(() => {
        const __bb_value = async () => {
          async function fetchJsonOrLoginWall(input, init) {
            ...
            const looksLikeHtml = contentType.toLowerCase().includes('text/html') || /^<(?:!doctype|html|head|body|title)(?:[\s>/]|$)/i.test(trimmed);
            if (looksLikeHtml) {
              return {
                __loginWall: true,
                status: r.status,
            ...
            try {
              ...
              throw new Error('JSON parse failed (status=' + r.status + ', body[0..50]=' + JSON.stringify(trimmed.slice(0, 50)) + '): ' + (err && err.message ? err.message : String(err)));
            }
          }
