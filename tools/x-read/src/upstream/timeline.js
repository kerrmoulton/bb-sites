...
cli({
  ...
  func: async (page, kwargs) => {
    ...
    const seen = new Set();
    let cursor = null;
    const visitedCursors=new Set();let emptyPages=0;
    ...
    for (let i = 0; i < MAX_PAGINATION_PAGES && allTweets.length < limit; i++) {
      ...
      if(nextCursor===cursor||visitedCursors.has(nextCursor)||emptyPages>=2){
        const error=new CommandExecutionError(emptyPages>=2?'TIMELINE_NO_PROGRESS: two pages added no new IDs':'TIMELINE_CURSOR_LOOP');error.partial=allTweets;throw error;
      }
      visitedCursors.add(nextCursor);cursor=nextCursor;
    }
